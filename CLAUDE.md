...
- `pod-encryption/` — cross-implementation fixtures and header vectors for the Pod encryption format (`spec` `pod-encryption.md`). Not RDF, so outside `fixtures/` and never seen by the SHACL runner. `negative/`, `accept/` and `vectors.json` are GENERATED by `scripts/gen_pod_encryption_vectors.py` (CI runs `--check`); `scripts/check_pod_encryption.py --cascade <cmd>` runs them against `cascade-cli` and ratchets against `pod-encryption/KNOWN_FAILURES.json`, which is separate from the top-level one. Positive fixtures are frozen bytes from the implementations: never regenerate them here. Passphrases are test-only.
